...
export default function SelectFileModal({
    ...
    async function fetchFiles() {
        ...
        try {
            ...
            setFiles([...(evidenceFiles || []), NO_EVIDENCE_FILE]);
            setLoading(false);
        } catch (error) {
            console.error("Error fetching files:", error);
            setFiles([NO_EVIDENCE_FILE]);
                            ...
                            {filteredFiles.length > 0 ? (
                                filteredFiles.map((file) => {
                                    ...
                                    return (
                                        <div
                                            key={file.id}
                                            className="flex items-center border-2 border-transparent border-dotted justify-between gap-3 py-2 px-3 rounded-lg hover:bg-blue-50 [&:has(input:checked)]:bg-blue-200/80
