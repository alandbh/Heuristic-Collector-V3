function getHeuristicAverage(
    ...
    players.forEach((player) => {
        ...
        const manyPlayers = showManyPlayers ? showManyPlayers.split("|") : null;
        let barColor;
        if (showManyPlayers) {
            barColor = "color_" + Number(manyPlayers.indexOf(player.slug) + 1);
        } else {
        ...
        if (isOverlapJourneys) {
            ...
            const filteredScoresArray = scoresArray.filter(
                (score) => score !== null
            );
            ...
            averageScoreValue = Number(
                (
                    filteredScoresArray.reduce((a, b) => a + b, 0) /
                    filteredScoresArray.length
                ).toFixed(1)
            );
