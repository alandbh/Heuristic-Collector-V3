...
export const MUTATION_CREATE_HEURISTIC = gql`
    mutation createHeuristic(
        $name: String!
        $heuristicNumber: String!
        $description: String!
        $projectId: ID!
        $groupId: ID!
        $journeyIds: [JourneyWhereUniqueInput!]
        $notApplicablePlayerIds: [PlayerWhereUniqueInput!]
    ) {
        createHeuristic(
            data: {
                name: $name
                ...
                journeys: { connect: $journeyIds }
                not_applicaple_players: { connect: $notApplicablePlayerIds }
            }
        ) {
            id
            heuristicNumber
...
export const MUTATION_PUBLISH_HEURISTIC = gql`
    mutation publishHeuristic($heuristicId: ID!) {
        publishHeuristic(where: { id: $heuristicId }, to: PUBLISHED) {
            id
        }
    }
`;
