    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            for (
                ...
                daysRemaining.push(totalHeuristics - index);
                // console.log("collectsByDate", index, totalHeuristics - index);
            }

            const pastDays = [];

            let allCollectionsRemaining =
                // allCollectionsAmount + allCollects.length; // Fake. It should be just allCollectionsAmount
                allCollectionsAmount;

            for (const date in collectsByDate) {
                if (date >= startDate) {
                    allCollectionsRemaining -= collectsByDate[date].length;
                    pastDays.push(allCollectionsRemaining);
                }
            }
